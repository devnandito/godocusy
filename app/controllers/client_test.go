...
func (c Client) List() revel.Result {
...
func (c Client) Search() revel.Result {
	...
	if first_name == "" && last_name == "" && document == ""{
		...
		res := DB.Where("ci LIKE ?", document+"%").Find(&clients)
		...
		res := DB.Where("first_name LIKE ?", first_name+"%").Find(&clients)
		err := res.Error
		...
		res := DB.Where("last_name LIKE ?", last_name+"%").Find(&clients)
		err := res.Error
		...
		res := DB.Where("last_name LIKE ? OR ci LIKE ?", last_name+"%", document+"%").Find(&clients)
		err := res.Error
		...
		res := DB.Where("first_name LIKE ? OR ci LIKE ?", first_name+"%", document+"%").Find(&clients)
		err := res.Error
		...
		res := DB.Where("first_name LIKE ? OR last_name LIKE ?", first_name+"%", last_name+"%").Find(&clients)
		err := res.Error
	...
	return c.Render(clients, h2, notFound)
}

func (c Client) ListPage(page int) revel.Result {
	h2 := "GO CRUD WITH POSTGRES - SHOW RECORDS"
	clients := []models.Client{}
	// q := DB.Order("id desc").Limit(15).Offset(1).Find(&clients)
	q := DB.Order("id desc").Find(&clients)
	err := q.Error
	p := paginator.New(adapter.NewGORMAdapter(q), 1)
	if page == 0 {
